...
export default function ProcessosModule({ user, userInfo, onBack, onFiltros, onKanban, onRelatorio, onAdminAlertas, onDashboard, onFuturos }) {
  ...
  const mudarStatus = async (p, novoStatus) => {
    const dados = { status: novoStatus, atualizadoEm: new Date().toISOString() };
    // Se concluído e tem viagemId, registrar pagamento na viagem
    if (novoStatus === "Concluído" && p.viagemId) {
      try {
        await updateDoc(doc(db, "tceduc_viagens", p.viagemId), {
          instutoriaPaga: true,
          instutoriaPagaEm: new Date().toISOString(),
          atualizadoEm: new Date().toISOString(),
        });
      } catch(e) { console.warn("Erro ao marcar instrutoria paga:", e); }
    }
    await updateDoc(doc(db, "processos", p.id), dados);
